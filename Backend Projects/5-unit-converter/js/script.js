...
const convert = (unit, value, from, to) => {
  if (unit === 'length') {
    convertedValue =
...
const unitConverter = (event, unit) => {
  event.preventDefault();

  if (!forms || !results) {
    forms = document.querySelector('.form-container');
  ...
  console.log(unit, number, fromUnit, toUnit);
  const convertedValue = convert(unit, number, fromUnit, toUnit);
  ...
  results.innerHTML = `<p> Results of your calculation </p> <p>${convertedValue.toFixed(
    2
  )} ${toUnit}</p>
      <button onclick="closeResults()">Reset</button>
`;
};
